...
internal object TargaReader {
    ...
    fun decode(buf: ByteArray): BufferedImage {
       ...
       // if (buf[2].toInt() == 0x02 && buf[16].toInt() == 0x20) { // uncompressed BGRA
        if (btoi(buf[2]) == 0x02 && btoi(buf[16]) == 0x20) {
            while (n > 0) {
                val b = read(buf)
                val g = read(buf)
                val r = read(buf)
          ...
          else if(btoi(buf[2]) == 0x02 && btoi(buf[16]) == 0x18){
            while (n > 0) {
                val b = read(buf)
                val g = read(buf)
                val r = read(buf)
            ...
            while (n > 0) {
                ...
                if (nb and 0x80 == 0) { // 0x80=dec 128, bits 10000000
                    ...
                    for (i in 0..nb) pixels[idx++] = v //added the -1 here
                }
                n -= nb + 1
            }
        }
