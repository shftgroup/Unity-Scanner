...
public class JTargaReader {
    ...
    public static BufferedImage decode(byte[] buf) throws IOException {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if(i==2)
            {
                System.out.println("Image Type: " + buf[i]);
            }
        }
        ...
        if (buf[2] == 0x02 && buf[16] == 0x20) { // uncompressed BGRA
            System.out.println("Reading uncomppressed bgra .tga");
            while (n > 0) {
                int b = read(buf);
                int g = read(buf);
                int r = read(buf);
            ...
            System.out.println("Reading uncomppressed bgr .tga");
            while (n > 0) {
                int b = read(buf);
                int g = read(buf);
                int r = read(buf);
            ...
            // RLE compressed
            System.out.println("Reading RLE compressed  .tga");
            while (n > 0) {
                int nb = read(buf); // num of pixels
                if ((nb & 0x80) == 0) { // 0x80=dec 128, bits 10000000

                    for (int i = 0; i <= nb; i++) {
                        //System.out.println("number of bytes: " + nb);
                        int b = read(buf);
                        int g = read(buf);
                        int r = read(buf);
                    ...
                    nb &= 0x7f;
                    //System.out.println("else bitwise and: " + nb);
                    int b = read(buf);
                    int g = read(buf);
                    int r = read(buf);
